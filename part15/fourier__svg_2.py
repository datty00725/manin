...
class Pi(Scene):
    def construct(self):
        ...
        #svg_path = "question.svg"  # SVGファイルのパスを指定
        ...
        for path in paths:
            for subpath in path.get_subpaths():
                ...
                complex_points = np.array([complex(*sp_mob.point_from_proportion(alpha)[:2]) for alpha in np.arange(0, 1, 1 / N)])
                # normalize them to fit in scene
                complex_points = (complex_points - np.mean(complex_points)) / np.max(abs(complex_points)) * 4

                # create my epicycles
                ec = FourierEpicyclesMObject(complex_points, num_coefs=10, speed_factor=1,

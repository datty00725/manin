...
class FourierTex(Scene):
    ...
    def construct(self):
        ...
        for i in [5,10,100]:
            ...
            tit = Emoji("u1f436", "svg").scale_to_fit_width(10)
            paths = self.get_svg_paths(tit)  # SVGパスを取得
            all_vectors_circles = []
            all_dots = []
            all_traces = []
            ...
            self.play(*[DrawBorderThenFill(vc) for vc in all_vectors_circles])  # ベクトルと円を一斉に描画
            self.wait()  # 一時停止
            ...
            for vc, trace in zip(all_vectors_circles, all_traces):
                vc.add_updater(lambda m, dt, trace=trace: vectorsUpdater(m, trace, dt))  # アップデータを追加
            ...
            vmobjects = [m for m in self.mobjects if isinstance(m, VMobject) and m is not gros_titre and m is not sub_titre]

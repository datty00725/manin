...
class FourierTex(Scene):
    ...
    def construct(self):
        filenam = "satoru_3.svg"

        # gros_titre = Tex("キルア").scale(5)
        sub_titre = VGroup(Tex("Who Am I?")).scale(5)
        ...
        for i in [5, 10, 100]:
            ...
            if j == 0:
                self.play(
                    *[FadeOut(vc) for vc in all_vectors_circles], FadeOut(box1)
                )  # ベクトルと円をフェードアウト
                # self.play(FadeOut(box1))
            elif j == 1:
                self.play(
                    *[FadeOut(vc) for vc in all_vectors_circles], FadeOut(box2)
                )  # ベクトルと円をフェードアウト
            else:
                self.play(
                    *[FadeOut(vc) for vc in all_vectors_circles], FadeOut(box3)
                )  # ベクトルと円をフェードアウト
